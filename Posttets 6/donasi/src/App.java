...
public class App {
    ...
    public static void TambahDonasi(ArrayList<Donasi> dataDonasi, Scanner input) {
        try {
            ...
            System.out.print("Rumah Sakit Penerima: ");
            String rumahSakitPenerima = input.nextLine();
            
            DonasiKesehatan donasi = new DonasiKesehatan(namaDonatur, jumlahDonasi, metodePembayaran, rumahSakitPenerima);
            
            // Implementasi validasi dan proses donasi
            if (donasi.validasiDonasi()) {
                donasi.prosesDonasi();
                dataDonasi.add(donasi);
                
                // Tampilkan informasi jumlah donasi kesehatan (menggunakan static method)
                System.out.println("Total Donasi Kesehatan yang telah dibuat: " + DonasiKesehatan.getJumlahDonasiKesehatan());
            }
    ...
    public static void UpdateDonasi(ArrayList<Donasi> dataDonasi, Scanner input) {
        try {
            ...
            if (nomor < 1 || nomor > dataDonasi.size()) {
                ...
                if (dataDonasi.get(nomor - 1) instanceof DonasiKesehatan) {
                    DonasiKesehatan donasi = (DonasiKesehatan) dataDonasi.get(nomor - 1);
                    ...
                    System.out.println("Data donasi bukan merupakan Donasi Kesehatan");
                }
            }
        } catch (Exception e) {
    ...
    public static void VerifikasiDonasi(ArrayList<Donasi> dataDonasi, Scanner input) {
        try {
            ...
            if (dataDonasi.get(nomor - 1) instanceof DonasiKesehatan) {
                DonasiKesehatan donasi = (DonasiKesehatan) dataDonasi.get(nomor - 1);
                // Proses verifikasi untuk DonasiKesehatan
                try {
                    if (donasi.validasiDonasi()) {
                        System.out.println("Donasi valid dan siap diverifikasi");
                    }
                } catch (Exception e) {
                    throw new Exception("Gagal memvalidasi donasi: " + e.getMessage());
                }
            } else {
                throw new Exception("Data donasi bukan merupakan Donasi Kesehatan");
            }
